...
def train_model(model_path, processed_file_path, window_size):
    ...
    try:
        ...
        model = gensim.models.Word2Vec(corpus_file=processed_file_path, vector_size=300, window=window_size, min_count=1,
                                       workers=20)
...
#
    ...
    # # Remove stopwords
    # df['clean_sent'] = df['clean_sent'].apply(
...
if __name__=='__main__':
    ...
    for window_size in [4, 6, ]:
        print(f'train with window size - {window_size}')
        train_model(hparams.w2v_model_path, hparams.w2v_input_path, window_size)